...
const getScore= async (score,timeResolved,nameStudent)=>{
    ...
    if(score<8){
        return new Error(`El Estudiante ${nameStudent} obtuvo:${score} y es un pena, y su padre es el negro Jose xdxdxd`);
    }
    return `El Estudiante ${nameStudent} obtuvo:${score}, por lo tanto ¡Bien hecho puerco!`;
}
const getReview= async ()=>{
    const studentOne=await getScore(students[0].score,students[0].timeResolved,students[0].fullName);
    const studentTwo=await getScore(students[1].score,students[1].timeResolved,students[1].fullName);
    const studentThree=await getScore(students[2].score,students[2].timeResolved,students[2].fullName);
    const studentFour=await getScore(students[3].score,students[3].timeResolved,students[3].fullName);
    try {
        const result=await Promise.all([studentOne,studentTwo,studentThree,studentFour]);
        console.log(result);
